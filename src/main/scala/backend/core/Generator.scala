...
object Generator {
  ...
  // }
  def generateFullBoard(): Option[Board] = {
    val shuffled = Random.shuffle(1 to 9)
    val baseBoard = Board.empty()

    // Заполняем первую строку перемешанными числами для начала
    val seededBoard = (0 until 9).foldLeft(baseBoard) { (b, col) =>
      b.updated(0, col, Filled(shuffled(col)))
    }

    // Решаем доску, чтобы получить полное решение
    Solver.solveAll(seededBoard, 1).headOption
  }
